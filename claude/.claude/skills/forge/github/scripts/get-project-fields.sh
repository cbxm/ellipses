...
if [ -z "$PROJECT_NUM" ]; then
  if [ -f ".claude/forge.json" ]; then
    PROJECT_NUM=$(jq -r '.project.number' .claude/forge.json)
    PROJECT_ORG=$(jq -r '.project.org' .claude/forge.json)
  fi
fi

if [ -z "$PROJECT_NUM" ]; then
  echo "Usage: get-project-fields.sh <project-number>" >&2
  echo "Or create .claude/forge.json with project settings" >&2
  exit 1
fi
