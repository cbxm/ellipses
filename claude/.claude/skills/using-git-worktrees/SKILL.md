...
4. Check for project-specific worktree agent

```bash
# Check if project has a worktree-setup agent
ls .claude/agents/worktree-setup.md 2>/dev/null
```

**If agent exists:** Invoke it instead of manual setup:
```
Task(subagent_type='worktree-setup', prompt='Run setup for worktree at <path>')
```

The project agent handles all project-specific setup (Redis, Prisma, etc.) and returns structured results. Skip to step 7 after agent completes.

**If no agent:** Continue with steps 5-6 below.

5. Auto-detect and run project setup (if no agent).
...
6. Run tests to ensure the worktree is clean (if no agent).
